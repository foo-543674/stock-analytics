...
impl std::fmt::Display for ValidationFailedField{
  fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
    write!(f, "{}:{}", self.name, self.keys.iter().map(|key| key.to_string()).collect::<Vec<_>>().join(","))
  }
}
...
impl ValidationFailure {
  ...
  pub fn merge(&self, other: &ValidationFailure) -> ValidationFailure {
    ValidationFailure {
      fields: self.fields.iter().fold(
        self.fields.clone(),
        |acc: Vec<ValidationFailedField>, field| {
          let same_name_fields: Vec<_> = other.fields.iter().filter(|f| f.name == field.name).collect();
          if same_name_fields.is_empty() {
            return acc;
          } else {
            let merged = same_name_fields.iter().fold(field.clone(), |acc: ValidationFailedField, f| acc.merge(f));
            return acc.into_iter().filter(|f| &f.name != &merged.name).chain(vec![merged.clone()]).collect();
          }
        }
      )
      .into_iter()
      .chain(other.fields.iter().filter(|f| !self.has_field(&f.name)).cloned())
      .collect()
    }
  }
...
impl std::fmt::Display for ValidationFailure {
  fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
    write!(f, "validation failure: {}", self.fields.iter().map(|field| field.to_string()).collect::<Vec<_>>().join("\n"))
  }
} 
...
#[cfg(test)]
mod tests {
  use crate::applications::validation::validation_message_keys::{duplicate, length_equals, required};
  use super::*;
  ...
  fn validation_failed_field_should_able_to_merge_keys() {
    ...
    assert_eq!(merged.to_string(), "name:validation.required(),validation.length_equals(10)");
  }
  ...
  fn validation_failure_should_display_fields() {
    ...
    assert_eq!(failure.to_string(), "validation failure: name:validation.required(),validation.length_equals(10)");
  }
