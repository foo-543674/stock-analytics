...
use crate::{
  applications::{
    ...
    }, common::id_generator::IdGenerator, errors::application_error::ApplicationError, validation::{
  ...
  domains::brand::{brand::{Brand, BrandCode, BrandId}, sector::SectorId},
  util::{
    str::StringExt, 
    ulid::StringExtForUlid, unempty_string::UnemptyString
  }
};

use super::brand_register_input::BrandRegisterInput;

pub struct BrandFactory {
...
impl BrandFactory {
  ...
  pub async fn create(&self, target: &BrandRegisterInput) -> Result<Brand, ApplicationError> {
    return self.base_validator.validate(target)
      ...
      .and_then(|sector| async {
        Ok(Brand {
          ...
          name: UnemptyString::from_string(&target.name),
          code: BrandCode::from_string(&target.code),
          sector,
        })
      }).await;
  }
...
mod tests {
  use super::*;
  use crate::applications::common::id_generator::MockIdGenerator;
  ...
  proptest! {
    #[test]
    fn brand_factory_should_return_error_when_name_is_empty(
      name in empty(),
    ...
    fn brand_factory_should_return_brand_when_all_fields_are_not_satisfy_above_conditions(
      ...
      sector_repository.expect_find_by_id().times(1).returning(|_| Box::pin(async { Ok(Some(Default::default())) }));
      let mut id_generator = MockIdGenerator::new();
      let sample_ulid = Ulid::new();
      id_generator.expect_generate().times(1).returning(move || BrandId::new(sample_ulid));
      ...
      assert_eq!(created.id, BrandId::new(sample_ulid));
      assert_eq!(created.name.value(), name);
      assert_eq!(created.code.value(), code);
      assert_eq!(created.sector, Default::default());
    }
  }
}
