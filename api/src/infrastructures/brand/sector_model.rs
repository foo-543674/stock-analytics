...
use crate::{domains::brand::sector::{Sector, SectorCode, SectorId}, util::unempty_string::UnemptyString};

use super::{
  category_model::Entity as CategoryModel, 
  sector_group_model::Entity as SectorGroupModel
};
...
impl Model {
  pub async fn into_domain(&self, connection: &DatabaseConnection) -> Result<Sector, sea_orm::error::DbErr> {
    let sector_group = self.find_related(SectorGroupModel).one(&*connection).await?.unwrap();
    let category = self.find_related(CategoryModel).one(&*connection).await?.unwrap();
    Ok(Sector {
